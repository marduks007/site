...
def reCAPTCHAValidation(token):
    '''
    :param token:
    :return: reCAPTCHA validation
    '''
    result = requests.post(
        'https://www.google.com/recaptcha/api/siteverify',data={
            'secret': settings.REPATCHA_PRIVATE_KEY,
            'response': token
        }
    )
    return result.json()
...
def RedirectParams(**kwargs):
    ...
    :param kwargs:
    :return: Used to append url parameters when redirecting users
    '''
...
class AjaxFormMixin(object):
    '''
    mixin to ajaxify django form - can be over written by calling form_valid
    '''
...
def Directions(*args,**kwargs):
        ...
        :param args:
        :param kwargs:
        :return: Handles directions from Google
        '''
